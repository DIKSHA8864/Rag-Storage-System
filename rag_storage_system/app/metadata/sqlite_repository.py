    ...
    def list_llm_usage_since(self, tenant_id: int, since: str, limit: int = 50000) -> list[dict]:
        ...
        with self._connect() as conn:
            rows = conn.execute(
                ...
                "WHERE tenant_id = ? AND created_at >= ? ORDER BY created_at LIMIT ?",
                (tenant_id, since, limit),
            ).fetchall()
        return [
            {**dict(r), "is_question": bool(r["is_question"]),
    ...
    def intake_funnel_counts(self, tenant_id: int, since: str) -> dict:
        with self._connect() as conn:
            funnel = conn.execute(
                ...
                WHERE (m.tenant_id = :t OR (s.matter_id = 0 AND :t = 1)) AND s.created_at >= :since
""",
            ...
            uploads = conn.execute(
                ...
                SELECT
                    (SELECT COUNT(*) FROM uploaded_inputs u
                     ...
                     WHERE (m.tenant_id = :t OR (s.matter_id = 0 AND :t = 1)) AND u.created_at >= :since) AS intake_uploads,
                    (SELECT COUNT(*) FROM matter_documents d WHERE d.tenant_id = :t AND d.created_at >= :since) AS case_documents
""",
                {"t": tenant_id, "since": since},
            ).fetchone()
